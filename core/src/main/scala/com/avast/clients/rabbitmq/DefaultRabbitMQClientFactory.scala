...
private[rabbitmq] object DefaultRabbitMQClientFactory extends LazyLogging {
  ...
  private def prepareManualConsumer[F[_]: ToTask, A: DeliveryConverter](
      consumerConfig: ConsumerConfig,
      channelFactoryInfo: RabbitMQConnectionInfo,
      channel: ServerChannel,
      blockingScheduler: Scheduler,
    ...
    new DefaultRabbitMQManualConsumer[F, A](name, channel, queueName, useKluzo, monitor, failureAction, finalBlockingScheduler)
  }
  ...
  private def bindQueues(channelFactoryInfo: RabbitMQConnectionInfo, channel: ServerChannel, consumerConfig: ConsumerConfig): Unit = {
    import consumerConfig.queueName
